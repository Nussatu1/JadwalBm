...
import { X, Check, Video, Users } from 'lucide-react';
...
export default function EventFormModal({ isOpen, onClose, initialData = null }) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={initialData ? 'Edit Jadwal Acara' : 'Tambah Acara Baru'}
      subtitle="Detail agenda liputan Bakid Multimedia"
      maxWidth="max-w-lg"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* 2. Kategori & Status (Custom Select) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input
              type="text"
              list="cf-category-suggestions"
              placeholder="Misal: Liputan, Pengajian, Wisuda"
              value={formData.kategori}
              onChange={e => setFormData({ ...formData, kategori: e.target.value })}
              className="w-full h-9 px-3 bg-white border border-[#E5E7EB] rounded-[6px] text-[#0B0C0E] text-[13px] focus:outline-none focus:border-[#F6821F] focus:ring-1 focus:ring-[#F6821F]"
            />
            <datalist id="cf-category-suggestions">
        ...
        {/* 3. Custom DatePicker & TimePicker */}
        ...
        <div className="pt-3 border-t border-[#E5E7EB] flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="h-8 px-3.5 bg-white border border-[#E5E7EB] hover:bg-[#F6F6F7] text-[#0B0C0E] font-medium rounded-[6px] text-[12px] transition-colors"
          >
            Batal
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="h-8 px-4 bg-[#F6821F] hover:bg-[#DB6E0F] active:bg-[#C25B08] text-white font-medium rounded-[6px] text-[12px] transition-colors disabled:opacity-50"
          >
